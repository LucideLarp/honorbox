...
surface, and the playbook's full table of contents. The `stand` and `terminal` themes are published whole in
this repo (`themes/`), so the code standard is checkable before you pay for
the rest. The failure catalogue the suite is built from is published in full, free,
