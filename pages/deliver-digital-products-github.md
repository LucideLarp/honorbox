---
title: Delivering digital products through GitHub — a practical guide
---

If your buyers are developers, GitHub is a delivery channel with properties no
download link has: per-buyer access control, revocation, updates that arrive
as `git pull`, and zero hosting to run. This guide covers doing it well —
useful with or without our tooling; the one section selling something is
marked.
...
- **Buyers need GitHub accounts.** For dev tools, themes, boilerplates,
  agent packs — fine, that's where the audience lives. For ebooks aimed at
  non-developers: wrong channel, use something else.
...
Collect the username as a **custom field** on your Stripe Payment Link (or
checkout form). After payment you have a `checkout.session` carrying the
username; something must read it and send the invite:
...
("invite within 30 minutes") so minute-4 emails don't happen.
...
- [Selling with Stripe Payment Links — the complete guide](./sell-with-stripe-payment-links.html)
- [Gumroad alternatives for developers](./gumroad-alternatives.html)
